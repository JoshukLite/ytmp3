...
def convert_all_files_to_mp3(working_dir, video_tempdir, image_tempdir, track_number=0):
    ...
    for audio in files:
        track_number += 1
        convert_to_mp3(audio, working_dir, image_tempdir, track_number)

    logging.info('Finished converting files to mp3')


def convert_to_mp3(audio_file, working_dir, cover_dir, part_number=None):
    ...
    audio_tags = {'album': 'vapor', 'track': part_number}

    # image_dir = os.path.join(os.getcwd(), img_dir)
